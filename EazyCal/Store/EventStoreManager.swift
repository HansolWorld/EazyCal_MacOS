...
class EventStoreManager: ObservableObject {
    ...
    func saveEvent(title: String, isAllDay: Bool, startDate: Date?, endDate: Date?, repeatDate: RepeatType, url: String?, notes: [String], calendar: EKCalendar) async throws {
        try await eventStore.saveEvent(title: title, isAllDay: isAllDay, startDate: startDate, endDate: endDate, repeatDate: repeatDate, url: URL(string: url ?? ""), notes: notes, calendar: calendar)
    }
